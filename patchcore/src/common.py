...
class NearestNeighbourScorer(object): # : Anomaly Scorer 
    def __init__(self, n_nearest_neighbours: int, nn_method=FaissNN(False, 4)) -> None:
        ...
        """
        self.feature_merger = ConcatMerger() # 입력이 여러 스케일/레이어의 특징 리스트일 때 하나의 2D 배열로 합치는 역할

        self.n_nearest_neighbours = n_nearest_neighbours # k-NN에서 k 값. 이미지(또는 패치) 이상도를 계산할 때 몇 개 이웃을 평균할지 결정
        self.nn_method = nn_method # 실제 최근접 이웃 검색 백엔드(default: FAISS wrapper)

        self.imagelevel_nn = lambda query: self.nn_method.run(
            n_nearest_neighbours, query
        )
        # 픽셀(패치) 단위 스코어/마스크가 필요할 때, 특정 서브 인덱스(예: 해당 클래스/클러스터 등)에서 top-1 NN을 찾는 용도
    ...
    def fit(self, detection_features: List[np.ndarray]) -> None:
        ...
        Coreset으로 고른 sample_features를 인덱싱하여 '빠른 k-NN 질의가 가능한 메모리 뱅크'를 만드는 과정
        ...
        self.nn_method.fit(self.detection_features) # FAISS 인덱스를 빌드, Memory Bank(Coreset으로 줄인 대표 패치들의 인덱스)
    ...
    def predict(
        ...
        1. 메모리 뱅크(지원 집합)에 대해 각 query vector의 k-NN 검색을 수행
        2. 반환된 query_distances는 보통 [M, k] (각 query별 k개 neighbor와의 거리)
        3. np.mean(..., axis=-1)로 이웃 거리의 평균을 내서 질의별 이상 점수를 계산
        (경우에 따라 max나 top-1을 사용하는 등의 변형도 가능)
        ...
        query_distances, query_nns = self.nn_method.run(
            self.n_nearest_neighbours, query_features
        )

        anomaly_scores = np.mean(query_distances, axis=-1)
        return anomaly_scores, query_distances, query_nns # 이미지/패치 레벨 이상 점수, k개의 neighbors와의 거리, k개 neighbor의 인덱스(memory bank 내의 위치)
    ...
    def save(
        ...
        """
        인덱스 파일(FAISS)과 merge된 특징(옵션)을 저장/복구하는 함수
        추후 재학습 없이 바로 추론 가능
        """
